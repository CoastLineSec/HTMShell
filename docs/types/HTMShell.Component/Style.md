...
- SVG, font, media, and generic data assets through CSS URLs;
...
Declared component raster and simple SVG images are consumed through HTML `<img src="resource:name">`, not CSS URLs. Required passed resources use `<img src="input:name">`. Resource-reference values cannot be inspected by selectors or used by `background-image`, masks, cursors, generated content, or font sources. See [component resources](Resource.md) and [resource-reference inputs](ResourceReferenceInput.md).
