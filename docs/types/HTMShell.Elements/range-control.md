...
`range-control` sets the average volume of one approved PipeWire node or one
ordered channel.
...
| `data-htm-action` | Matching `pipewire.audio.set_volume` or `pipewire.audio.set_channel_volume` |
...
Inside `pipewire.nodes` then `item.channels`, the required binding/action pair
is `item.volume` with `pipewire.audio.set_channel_volume`. The current channel
and parent node are implicit. `data-htm-target` is forbidden. Channel indexes
and raw node IDs cannot target the control.
...
Control state is `idle`, `pending`, `failed`, or `unavailable`. A failed control
returns to the latest confirmed value. Author `disabled` is never removed. A
channel-layout replacement cancels the old interaction and makes its control
identity stale.
...
- [Channel controls](../HTMShell.Services.PipeWire/ChannelControls.md)
