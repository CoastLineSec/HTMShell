...
| `data-htm-source` | `upower.devices` or `power_profile.holds` |
...
The current limits are 32 repeats per document, 256 items per repeat, 64 registered descendants per template, depth 32, 4,096 cloned nodes per repeat, and 16,384 cloned nodes per document.
