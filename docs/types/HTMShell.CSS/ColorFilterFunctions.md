...
`blur()` and `drop-shadow()` are documented separately and may be ordered with these color functions. Color, blur, and drop-shadow combinations execute natively through Vello with explicit alpha-representation transitions. GPU failure falls back to complete CPU rendering of the ordered list; no prefix or suffix is split across backends. `backdrop-filter`, URL filters, arbitrary matrices, and author shaders are unsupported.
