...
CPU headless and CPU Wayland presentation use the same reference compositor. The optional experimental Vello path executes all ten functions with bounded native GPU effect layers. It uses the same encoded-sRGB matrices, function order, repeated stages, per-stage clamping, Gaussian parameters, three-box widths, shadow alpha semantics, and transparent-edge behavior as the CPU reference.

Successful Vello foreground-effect frames do not use CPU frame rasterization, GPU readback, or shared-memory presentation. Identity-only lists skip unnecessary GPU effect work. The GPU pipeline converts straight RGBA to premultiplied RGBA before a spatial stage and converts back only when a later color stage or Vello composition requires it. Consecutive premultiplied spatial stages avoid redundant conversions.

Native drop shadow extracts current-stage alpha into a bounded exact-byte alpha-carrier texture. Source RGB does not affect the mask. The shared GPU blur passes process the mask, manual transparent-zero bilinear sampling preserves fractional physical offsets, encoded-sRGB tinting applies color alpha once, and premultiplied source-over keeps the current source above the shadow. GPU failure requests complete CPU-frame fallback for the entire ordered list; one list is never split across backends. The GPU effect layer uses canonical `Rgba8Unorm`, permits at most 16 functions, and follows the same 4,096 pixel dimension, 64 MiB image, 256 MiB per-surface, and nesting limits.

Damage-limited Vello replay expands each 64 by 64 pixel tile by the complete ordered spatial dependency plus the existing two-pixel antialias guard. Blur contributes its physical support. Drop shadow contributes mask blur support, physical offset reach, and fractional sampling reach. Only the tile core enters persistent backing. The guarded replay area, not just the core area, controls the 30 percent partial-render threshold. Unsafe transformed spatial layers and excessive guarded areas use a complete GPU render. Backing-to-surface conversion remains full target.

## Current boundary

All ten functions in this bounded foreground profile render through the CPU reference compositor and have experimental native Vello execution. Vello remains optional and experimental. GPU failures use complete CPU fallback. URL filters, multiple drop shadows in one list, spread, inset shadows, and `backdrop-filter` are not supported.
